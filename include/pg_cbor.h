...
typedef enum {
	...
	CborFlagsMajorTypeMaskEncoded = 7 << 5, // mask for major type id
...
typedef enum {
	...
	CborIterStateValue,
	CborIterStateObject,
	CborIterStateObjectKey,
	CborIterStateObjectValue,
	CborIterStateArray,
	CborIterStateArrayValue,
	CborIterStateEnd,
} CborIterState;
...
typedef enum {
	...
	CborIteratorTokenElem,
	CborIteratorTokenBeginArray,
	CborbIteratorTokenEndArray,
	CborbIteratorTokenBeginObject,
	CborbIteratorTokenEndObject,
} CborIteratorToken;
...
typedef struct CborIterator {
	/* Container being iterated */
	CborData container;
	CborData current;

	uint32 nElems; /* Number of elements in children*/
	...
	struct CborIterator *parent;
} CborIterator;

bool data_is_cbor(const uint8_t *, size_t);

uint8_t CborDataReadUnsigned(CborData *);
uint16_t CborDataReadUnsigned16(CborData *);
uint32_t CborDataReadUnsigned32(CborData *);
uint64_t CborDataReadUnsigned64(CborData *);

uint64_t CborDataReadUnsignedValue(CborData *, uint8_t);

CborIterator *CborIteratorInit(const uint8_t *, size_t);
